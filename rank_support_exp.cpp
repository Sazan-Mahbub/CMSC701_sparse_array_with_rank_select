...
//    vector<int> _bitvect_sizes_ = {10000, 100000, 1000000};

    vector<double> jcb_time;
    vector<double> nrml_time;
    vector<double> jcb_ovrld;
    vector<double> nrml_ovrld;
...
//        cout << "zero_fraction\t" << zero_fraction << endl << endl;
        ...
        for (int i=0; i < 1000000; i+=100){
            sa.rs_obj.rank1_jacobson(i%bitvec_size);
        }
        auto toc = Clock::now();

        double total_time = (double) duration_cast<nanoseconds>(toc - tic).count()/1000;
        cout << "Jacobson's method:\tElapsed time:\t" << total_time << " microseconds" <<  endl;
        cout << "Jacobson's method:\tOverhead:\t" << sa.rs_obj.overhead() << endl;

        jcb_time.push_back(total_time);
        jcb_ovrld.push_back(sa.rs_obj.overhead());


        /// precomputed and stored all ranks

        tic = Clock::now();
        // run analysis
        for (int i=0; i < 1000000; i+=100){
            sa.rs_obj.rank1(i%bitvec_size);
        }
        toc = Clock::now();

        total_time = (double) duration_cast<nanoseconds>(toc - tic).count()/1000;
        cout << "Store-all-select1:\tElapsed time:\t" << total_time << " microseconds" <<  endl;
        cout << "Store-all-select1:\tOverhead:\t" << sa.rs_obj.overhead_more_space() << endl;

        nrml_time.push_back(total_time);
        nrml_ovrld.push_back(sa.rs_obj.overhead_more_space());
    ...
    cout << "nrml_time= [";
    for (auto x: nrml_time){
        cout << x <<  ", ";
    }
    cout << "]" << endl;

    cout << "nrml_ovrld= [";
    for (auto x: nrml_ovrld){
        cout << x <<  ", ";
    }
    cout << "]" << endl;
