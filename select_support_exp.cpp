...
//        cout << "zero_fraction\t" << zero_fraction << endl << endl;
        ...
        /// select1 operation O(1) time, O(mlongn) space

        auto tic = Clock::now();
        // run analysis
        for (int i=0; i < 1000000; i+=100){
            sa.ss_obj.select1(i/(sa.rs_obj.n+1));
        }
        auto toc = Clock::now();
        ...
        cout << "Store-all-select1:\tOverhead:\t" << sa.ss_obj.overhead() << endl;


        nrml_time.push_back(total_time);
        nrml_ovrld.push_back(sa.ss_obj.overhead());
